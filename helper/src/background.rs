...
mod native {
    ...
    use ::windows::Win32::UI::Accessibility::{
        IUIAutomation, IUIAutomationInvokePattern,
    ...
    use crate::target_guard::native::{check_security, verify_element_ancestry};
    ...
    pub fn execute_background_act(
        ...
        let element = snapshot
            .elements
            .get(&request.handle)
            .ok_or_else(|| format!("DESKTOP_REF_STALE: Element handle {} not found in snapshot", request.handle))?;
        ...
        match request.op.as_str() {
            "invoke" => unsafe {
                let pattern = element
                    ...
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: InvokePattern not supported".to_string())?;
                let invoke_pat: IUIAutomationInvokePattern = pattern
                    .cast()
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: Cannot cast to InvokePattern".to_string())?;
                invoke_pat
                    .Invoke()
            ...
            "setValue" => unsafe {
                let val_str = request
                    .value
                    .as_deref()
                    .ok_or_else(|| "DESKTOP_PATTERN_UNSUPPORTED: setValue requires value argument".to_string())?;

                if val_str.encode_utf16().count() > 65536 {
                    return Err("DESKTOP_PATTERN_UNSUPPORTED: Value exceeds 65536 UTF-16 code units".to_string());
                }

                let is_password = element.CurrentIsPassword().map(|b| b.as_bool()).unwrap_or(false);
                if is_password {
                    return Err("DESKTOP_INPUT_REFUSED: setValue refused on password field".to_string());
                }

                let pattern = element
                    .GetCurrentPattern(UIA_ValuePatternId)
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: ValuePattern not supported".to_string())?;
                let value_pat: IUIAutomationValuePattern = pattern
                    .cast()
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: Cannot cast to ValuePattern".to_string())?;

                let read_only = value_pat.CurrentIsReadOnly().map(|b| b.as_bool()).unwrap_or(false);
                if read_only {
                ...
                value_pat
                    .SetValue(&bstr)
                    .map_err(|err| format!("DESKTOP_PATTERN_UNSUPPORTED: SetValue failed: {err}"))?;
            },
            "select" => unsafe {
                let pattern = element
                    .GetCurrentPattern(UIA_SelectionItemPatternId)
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: SelectionItemPattern not supported".to_string())?;
                let select_pat: IUIAutomationSelectionItemPattern = pattern
                    .cast()
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: Cannot cast to SelectionItemPattern".to_string())?;
                select_pat
                    .Select()
            ...
            "toggle" => unsafe {
                let pattern = element
                    ...
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: TogglePattern not supported".to_string())?;
                let toggle_pat: IUIAutomationTogglePattern = pattern
                    .cast()
                    .map_err(|_| "DESKTOP_PATTERN_UNSUPPORTED: Cannot cast to TogglePattern".to_string())?;
                toggle_pat
                    .Toggle()
            ...
            unknown => return Err(format!("DESKTOP_PATTERN_UNSUPPORTED: Unknown action operation: {unknown}")),
        }
