...
test('dashboard keeps Remote Access independent and every section defaults expanded without forcing reopened state',()=>{assert.match(js,/makeDashboardCollapsible/);assert.match(js,/dashboard-section/);assert.match(js,/details\.open=true/);assert.match(js,/onboarding\.open=true/);assert.match(js,/!onboarding\.classList\.contains\(['"]dashboard-section['"]\)/);assert.doesNotMatch(js,/if\(onboarding\)\{onboarding\.classList\.add\(['"]dashboard-section['"]\);onboarding\.open=true;\}/);assert.doesNotMatch(js,/remote-in-onboarding/);assert.doesNotMatch(js,/body\.prepend\(remote\)/);assert.doesNotMatch(js,/onboarding\.open=false/);});
...
test('request drawer decorates newly rendered critical cards immediately without mutation loops',()=>{assert.match(js,/latestApprovals/);assert.match(js,/decorateRequestDrawer\(latestApprovals\)/);assert.match(js,/detail\.innerHTML!==markup/);assert.match(js,/textContent!==/);});
