...
function readdirSafe(target){try{return readdirSync(target,{withFileTypes:true});}catch{return null;}}
...
test('active code and docs contain no Linker migration compatibility',()=>{
  ...
  for(const root of roots){
    for(const file of filesUnder(root)){
      ...
      for(const pattern of forbidden)if(pattern.test(text)){offenders.push(`${file}: ${pattern}`);break;}
    }
  }
