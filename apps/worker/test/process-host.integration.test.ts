...
async function until(predicate:()=>boolean,ms=5000){const end=Date.now()+ms;while(Date.now()<end){if(predicate())return;await new Promise(r=>setTimeout(r,50));}throw new Error('condition not met within '+ms+'ms');}
...
test('managed process has bounded logs and explicit stop', async () => {
  ...
  const processInfo = runtime.start(
    { executable: process.execPath, args: ['-e', 'console.log("hello") ; setTimeout(()=>{},1000)'], env: {} },
    process.cwd(),
    'stop-with-aevra',
  );
...
test('keep-running uses a detached process-host with an ownership marker and redacted persisted log', async () => {
  ...
  const value = runtime.start(
    { executable: process.execPath, args: ['-e', 'console.log(process.env.TEST_SECRET); setTimeout(()=>{},1000)'], env: { TEST_SECRET: 'super-secret-value' } },
    process.cwd(),
    'keep-running',
  ...
  await until(()=>{try{const t=readFileSync(value.logPath,'utf8');return /REDACTED/.test(t)&&!/super-secret-value/.test(t);}catch{return false;}});
