...
function displayValue<T>(row: T, column: Column<T>): React.ReactNode {
  if (column.render) return column.render(row);
  const value = rawValue(row, column);
  return column.dateTime ? formatLocalDateTime(value) : String(value ?? '');
}
...
export function DataTable<T>({
  ...
  const [size, setSize] = useState<5 | 10 | 25 | 50 | 100>(pageSize);
