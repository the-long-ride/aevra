...
export function installApiFixtures(options: FixtureOptions = {}) {
  ...
  const routes = new Map<string, unknown>([
    ...
    ['/api/onboarding', { completed, completedSections: completed ? ['remote-access', 'connect-ai', 'workspace', 'try-aevra'] : [] }],
    ...
    ['/api/dashboard/runtime', {
      ...
      uptimeSeconds: 100,
      pending: { total: 0 },
      stats: { sessions: 1, workspaceLeases: 1, processes: 0, openChanges: 0, toolCalls: 3, avgToolLatencyMs: 10, connectors: 1 },
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (method !== 'GET') return json({ ok: true, result: { hostname: 'aevra.example.com' }, token: 'secret-once' });
