...
test('approved workspace read access survives a fresh MCP security session without another request', async () => {
  ...
  const workspaceTickets = f.approvals
    .list()
    .filter(
      (ticket) =>
        ticket.operation.family === 'workspace:select' && ticket.workspaceId === f.aevra.id,
    );
...
test('pending workspace access request is reused across MCP reconnect before local Allow', async () => {
  ...
  f.sessions.disconnect(first.id);
  const second = f.sessions.create(identity());
  const retriedBeforeAllow = (await f.tools.call(second.id, 'workspace_select', {
    workspace: 'Aevra',
  })) as any;
  ...
  const retriedAfterAllow = (await f.tools.call(second.id, 'workspace_select', {
    workspace: 'Aevra',
  })) as any;
...
test('another workspace in the same OAuth authorization needs its own approval once', async () => {
  ...
  const selected = (await f.tools.call(session.id, 'approval_wait', {
  ...
  const back = (await f.tools.call(reconnected.id, 'workspace_select', {
    workspace: 'Aevra',
  })) as any;
  assert.equal(back.status, 'selected');
  const tickets = f.approvals
    .list()
    .filter((ticket) => ticket.operation.family === 'workspace:select');
