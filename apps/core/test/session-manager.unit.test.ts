...
function fixture(){
  ...
  db.raw().prepare("INSERT INTO workspaces(id,name,host_root,created_at,updated_at) VALUES('w','W','/tmp',?,?)").run(now,now);
  db.raw().prepare("INSERT INTO workspaces(id,name,host_root,created_at,updated_at) VALUES('w2','W2','/tmp/2',?,?)").run(now,now);
...
test('Core generates fresh session IDs and refreshes lease inactivity',()=>{
  const db=AevraDatabase.open(':memory:');
  const profiles=new CapabilityProfileService(db.raw());
  db.raw().prepare("INSERT INTO workspaces(id,name,host_root,created_at,updated_at) VALUES('w','W','/tmp',?,?)").run(new Date().toISOString(),new Date().toISOString());
  profiles.mapActor('a','w','read-only','auto');
  let now=new Date('2026-01-01T00:00:00Z');
  const mgr=new SessionManager(new SessionRepository(db.raw()),profiles,30*60_000,{now:()=>now});
  const id={subject:'s',actor:'a',issuer:'i',audience:'x',expiresAt:'x'};
  const a=mgr.create(id),b=mgr.create(id);
  assert.notEqual(a.id,b.id);
  const admitted=mgr.admitWorkspace(a.id,'w');assert.equal(admitted.status,'admitted');
  const first=(admitted as any).lease.expiresAt;
  now=new Date(now.getTime()+10*60_000);mgr.touch(a.id);
...
test('unknown actor workspace requires admission approval',()=>{
  ...
  const s=manager.create({subject:'s',actor:'a',issuer:'i',audience:'x',expiresAt:'x'});
...
test('OAuth connection workspace grant restores a fresh lease after MCP reconnect',()=>{
  ...
  const secondLease=manager.activeLease(second.id);assert.ok(secondLease);
...
test('OAuth connection workspace grant does not cross to another authorization subject',()=>{
  const {db,manager}=fixture();
  const first=manager.create({subject:'oauth_grant_one',actor:'oauth:ChatGPT',issuer:'i',audience:'x',expiresAt:'x'});
  manager.grantConnectionWorkspace(first.id,'w','read-only');
  manager.disconnect(first.id);
  const other=manager.create({subject:'oauth_grant_two',actor:'oauth:ChatGPT',issuer:'i',audience:'x',expiresAt:'x'});
