...
function fixture() {
  ...
  return {
    service: new ProcessService(sessions, workspaces, worker, repo),
    stored,
    updated,
    records,
  };
...
test('managed process name is persisted and local rows expose workspace name', async () => {
  const f = fixture();
  await f.service.start(
    'ses_1',
  ...
  assert.equal(f.stored[0]?.name, 'Vite dev server');
  const local = f.service.listLocal()[0] as any;
  assert.equal(local.name, 'Vite dev server');
  assert.equal(local.workspace_name, 'Aevra');
});
