...
const identity={subject:'sub',actor:'actor@example.test',issuer:'https://issuer.test',audience:'aud',expiresAt:new Date(Date.now()+60000).toISOString()};

test('MCP initialize creates server-owned session and unknown client session is rejected', async()=>{
  const db=AevraDatabase.open(':memory:');const sessions=new SessionManager(new SessionRepository(db.raw()),new CapabilityProfileService(db.raw()));
  const verifier={async verifyRequest(){return identity}} as any;
  const service={async call(){return{ok:true}}} as any;
  const server=new McpIngressServer('127.0.0.1',0,verifier,undefined,()=>false,{sessions,service} as any);await server.start();
