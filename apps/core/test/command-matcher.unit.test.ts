...
async function policy(){try{return await import('../src/policy/command-matcher.js');}catch{return{} as any;}}

test('command matcher wildcards paths filenames and parameter values',async()=>{const {commandPermissionMatcher}=await policy();assert.equal(typeof commandPermissionMatcher,'function','commandPermissionMatcher must exist');
...
test('command matcher keeps option names and never stores shell script text',async()=>{const {commandPermissionMatcher}=await policy();assert.equal(typeof commandPermissionMatcher,'function','commandPermissionMatcher must exist');
