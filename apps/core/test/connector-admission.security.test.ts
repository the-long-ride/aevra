...
async function withServer(
  fn: (server: McpIngressServer, repo: ConnectorRepository) => Promise<void>,
) {
  ...
  const server = new McpIngressServer(
    ...
    undefined,
    () => false,
    { sessions, service: { call: async () => ({ ok: true }) } } as any,
    {
      verify: async (t: string) => {
        const r = repo.findByToken(t);
        return r
          ? ({
              kind: 'admitted',
              identity: {
                actor: `connector:${r.name}`,
                subject: r.id,
  ...
  await server.start();
  try {
    await fn(server, repo);
  } finally {
    await server.close();
    db.close();
  }
}
test('token brute force gets uniform 401 with no body differences', async () => {
  await withServer(async (server) => {
    const responses: string[] = [];
    for (const probe of ['a', 'ab', 'abc', 'abcdefghij1234567', 'zzzzzzzzzzzzzzzzzzzz']) {
      const r = await fetch(`${server.url()}/mcp/${probe}`, {
        method: 'POST',
...
test('safe mode blocks connector admission like /mcp', async () => {
  ...
  const server = new McpIngressServer(
    ...
    undefined,
    () => true,
    { sessions, service: { call: async () => ({}) } } as any,
    { verify: async () => ({ kind: 'denied' }) as const },
  );
  await server.start();
  const r = await fetch(`${server.url()}/mcp/${token}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {} }),
  });
...
test('remote MCP tool surface exposes no connector management', async () => {
  ...
  assert.equal(
    names.some((n) => n.startsWith('connector')),
    false,
  );
});
test('exhausted rate limit returns 429 while below-threshold responses stay uniform', async () => {
  ...
  const server = new McpIngressServer(
    ...
    { sessions, service: { call: async () => ({}) } } as any,
    {
      verify: async (t: string, ip: string) => {
  ...
  const bodies: string[] = [];
  for (let i = 0; i < 5; i++) {
    const r = await fetch(`${server.url()}/mcp/wrong-token-${i}`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {} }),
    });
  ...
  assert.ok(
    bodies.every((x) => x === bodies[0]),
    'below threshold: uniform 401 bodies',
  );
  const throttled = await fetch(`${server.url()}/mcp/wrong-token-9`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {} }),
  });
