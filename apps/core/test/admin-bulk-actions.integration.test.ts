...
test('bulk permission route rejects persistent allow records for critical command families',async()=>{
  const rules:any[]=[];
  const server=new AdminServer('127.0.0.1',0,()=>({core:'running'}),{bootstrap,api:{permissions:{upsert:(rule:any)=>rules.push(rule)}} as any});
  await server.start();
  const response=await request(server,'/api/permissions/bulk',{method:'POST',body:JSON.stringify({effect:'allow',scope:'global',actors:['oauth:ChatGPT'],capabilities:['commands.run'],matcher:'git:push:force'})});
