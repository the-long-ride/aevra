...
test('bulk permission route expands capabilities into ordinary rule records', async () => {
  ...
  const response = await request(server, '/api/permissions/bulk', {
    ...
    body: JSON.stringify({
      ...
      capabilities: ['files.write', 'commands.run'],
    }),
  });
  ...
  assert.deepEqual(
    rules.map((rule) => [rule.actor, rule.capability, rule.scope, rule.matcher]),
    [
...
test('bulk permission route rejects persistent allow records for critical command families', async () => {
  ...
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap,
    api: { permissions: { upsert: (rule: any) => rules.push(rule) } } as any,
  });
  await server.start();
  const response = await request(server, '/api/permissions/bulk', {
    method: 'POST',
  ...
  const value = (await response.json()) as any;
  assert.equal(value.error.code, 'CRITICAL_RULE_FORBIDDEN');
  await server.close();
});
...
test('revoke-others keeps OAuth and static connector MCP sessions plus the current admin session', async () => {
  ...
  let keptAdmin: string | undefined;
  const sessions = {
    list: () => [
      { id: 'access_1', actor: 'access:user@example.com' },
      { id: 'oauth_1', actor: 'oauth:ChatGPT' },
      { id: 'connector_1', actor: 'connector:Claude' },
    ],
    revoke: (id: string) => revoked.push(id),
  };
  const localBootstrap = {
    validateSession: (value: string | undefined) => value === 'keep-me',
    revokeAllExcept: (value: string | undefined) => {
      keptAdmin = value;
      return { revoked: 2, preserved: 1 };
    },
  };
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap: localBootstrap as any,
    api: { sessions, bootstrap: localBootstrap } as any,
  });
  await server.start();
  const response = await request(server, '/api/sessions/revoke-others', {
    method: 'POST',
    body: '{}',
  });
  assert.equal(response.status, 200);
...
test('bulk mutations remain disabled while Aevra is in safe mode', async () => {
  ...
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap,
    api: { safeMode: () => true, permissions: { upsert: (rule: any) => rules.push(rule) } } as any,
  });
  await server.start();
  const response = await request(server, '/api/permissions/bulk', {
    method: 'POST',
