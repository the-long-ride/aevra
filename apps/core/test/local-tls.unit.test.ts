...
test('Windows normal-start path can trust the generated certificate in CurrentUser Root', {skip:process.platform!=='win32'}, async(t)=>{
...
`;
  ...
  t.after(()=>{spawnSync('powershell.exe',['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-Command',script],{encoding:'utf8',env:{...env,AEVRA_TLS_TEST_REMOVE:'1'}});});
  const found=spawnSync('powershell.exe',['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-Command',script],{encoding:'utf8',env});
