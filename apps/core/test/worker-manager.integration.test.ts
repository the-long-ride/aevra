...
test('worker manager includes child stderr when bootstrap exits',async()=>{
  ...
  const manager=new WorkerManager(workerSocketPathForPlatform(dir),path.join(dir,'process-logs'),{entryPath:entry} as any);
  await assert.rejects(manager.start(),/worker-bootstrap-boom/);
  await manager.close();
});

test('Windows worker endpoint is unique per core process',()=>{
  assert.equal(workerSocketPathForPlatform('C:\\state','win32',4242),'\\\\.\\pipe\\aevra-worker-4242');
  assert.equal(workerSocketPathForPlatform('/tmp/aevra','linux',4242),path.join('/tmp/aevra','worker.sock'));
});
...
test('worker manager starts the packaged worker from its module location and fully stops it',async()=>{
  ...
  const manager=new WorkerManager(workerSocketPathForPlatform(dir),path.join(dir,'process-logs'));
