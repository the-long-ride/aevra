...
test('approve arms frozen operation but does not execute until resume', async () => {
  const { db, svc } = make();
  let runs = 0;
  ...
  svc.approve(r.requestId, 'session');
  assert.equal(runs, 0);
  await svc.resume(
    r.requestId,
    async () => ({ ok: true }),
...
test('context change blocks execution', async () => {
  const { db, svc } = make();
  let runs = 0;
  const r = await svc.request({
    actor: 'a',
  ...
  await assert.rejects(
    () =>
      svc.resume(
        r.requestId,
        async () => ({ ok: false, reason: 'workspace changed' }),
        async () => ++runs,
      ),
...
test('critical approvals reject persistent scopes and allow once', async () => {
  const { db, svc } = make();
  const r = await svc.request({
    actor: 'oauth:ChatGPT',
    sessionId: 's',
    workspaceId: 'w',
    operation: { family: 'danger', capability: 'commands.run', risk: 'CRITICAL', argsHash: 'h' },
    payload: {
      tool: 'capability_request',
      requestedCapability: 'commands.run',
      permissionMatcher: 'danger',
    },
    expectedState: {},
    risk: 'CRITICAL',
  });
  assert.throws(() => svc.approve(r.requestId, 'session'), /one-time/i);
...
test('default static connector workspace approval is normalized to read-only', async () => {
  ...
  const r = await svc.request({
    ...
    payload: { tool: 'workspace_select', workspaceId: 'w', profileId: 'developer' },
    expectedState: { workspaceId: 'w' },
    risk: 'MEDIUM',
  });
  const approved = svc.approve(r.requestId, 'once');
  assert.equal((approved.payload as any).profileId, 'read-only');
