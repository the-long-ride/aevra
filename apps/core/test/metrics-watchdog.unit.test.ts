...
test('metrics snapshot aggregates calls and durations',()=>{
  ...
  m.record('file_read',10);m.record('file_read',30);m.record('aevra_status',5);
  const snap=m.snapshot();
  assert.deepEqual(snap.find(x=>x.tool==='file_read'),{tool:'file_read',calls:2,totalMs:40,avgMs:20});
  assert.equal(snap.length,2);
  m.reset();
  assert.deepEqual(m.snapshot(),[]);
});
test('tunnel watchdog records status and signals drops',async()=>{
  let reachable=true;const drops:string[]=[];
  const wd=new TunnelWatchdog(async()=>({reachable,message:reachable?'reachable':'HTTP 403'}),10,msg=>drops.push(msg));
  wd.start();
