...
import {mkdtempSync,mkdirSync,writeFileSync,rmSync} from 'node:fs'; import {tmpdir} from 'node:os'; import path from 'node:path';
...
function tree(){
  ...
  writeFileSync(path.join(home,'.agents','skills','alpha','SKILL.md'),'---\nname: Alpha\ndescription: Does alpha things\n---\n# Alpha\nUse alpha.');
  ...
  writeFileSync(path.join(ws,'.agents','skills','alpha','SKILL.md'),'---\nname: Alpha\ndescription: Workspace alpha\n---\nworkspace alpha');
...
test('list merges user and workspace skills with fallback names',()=>{
  const {home,ws}=tree();const svc=new SkillsService(home);
  ...
  assert.equal(names.find(s=>s.source==='workspace'&&s.name==='Alpha')!.description,'Workspace alpha');
...
test('read returns SKILL.md, supporting files list, and file content',()=>{
  ...
  assert.match(main.content,/Use alpha/);assert.deepEqual(main.files,[]);
...
test('read blocks traversal and size caps',()=>{
  const {home,ws,base}=tree();const svc=new SkillsService(home);
  writeFileSync(path.join(home,'.agents','escape.txt'),'x'); // '../../escape.txt' from a skill dir resolves to .agents/
  assert.throws(()=>svc.read('user','Alpha',ws,'../../escape.txt'),(e:any)=>e.code==='SKILL_PATH_ESCAPE');
  writeFileSync(path.join(home,'.agents','skills','beta','big.md'),'x'.repeat(256*1024+1));
  assert.throws(()=>svc.read('user','beta',ws,'big.md'),(e:any)=>e.code==='SKILL_FILE_TOO_LARGE');
  assert.throws(()=>svc.read('user','nope',ws),(e:any)=>e.code==='SKILL_NOT_FOUND');
  assert.throws(()=>svc.read('workspace','Alpha',null),(e:any)=>e.code==='SKILL_NOT_FOUND');
});
test('instructions merge user global then workspace, with CLAUDE.md fallback',()=>{
  const {home,ws,base}=tree();const svc=new SkillsService(home);
  const r=svc.instructions(ws);
  assert.deepEqual(r.instructions.map(x=>x.source),['user','workspace']);
  assert.match(r.instructions[1]!.content,/workspace instructions/);
  rmSync(path.join(ws,'AGENTS.md'));writeFileSync(path.join(ws,'CLAUDE.md'),'claude fallback');
  ...
  assert.deepEqual(empty.instructions,[]);assert.ok(empty.note);
});
test('oversized instruction file throws SKILL_FILE_TOO_LARGE',()=>{
  ...
  assert.throws(()=>svc.instructions(null),(e:any)=>e.code==='SKILL_FILE_TOO_LARGE');
});
