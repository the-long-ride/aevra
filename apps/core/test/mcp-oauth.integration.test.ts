...
async function fixture() {
  ...
  const oauth = new AevraOAuthService(oauthRepo, {
  ...
  const server = new McpIngressServer(
    ...
    async (_req, res, identity) => {
      identities.push(identity.actor);
      res.statusCode = 200;
      res.setHeader('content-type', 'application/json');
      res.end(JSON.stringify({ ok: true, actor: identity.actor }));
    },
    () => false,
    undefined,
    connectors,
...
test('MCP OAuth discovery and 401 challenge advertise protected resource metadata', async () => {
  ...
  assert.equal(
    ((await auth.json()) as any).registration_endpoint,
...
test('DCR + local approval + PKCE token exchange authenticates canonical /mcp', async () => {
  ...
  authorize.searchParams.set('resource', 'https://mcp.example.com/mcp');
  authorize.searchParams.set('code_challenge', challenge);
  authorize.searchParams.set('code_challenge_method', 'S256');
  authorize.searchParams.set('state', 'state123');
  ...
  assert.ok(requestId);
  const pendingRequest = f.oauth.listPendingAuthorizations().find((item) => item.id === requestId);
  assert.ok(pendingRequest?.pairingCode);
  const expectedCommand = `aevra oauth approve ${requestId} --code ${pendingRequest.pairingCode}`;
  const pageReady =
    html.includes(expectedCommand) &&
    /id="copy-command"/.test(html) &&
    /navigator\.clipboard\.writeText/.test(html) &&
    /JetBrains Mono/.test(html) &&
    /data-aevra-oauth/.test(html);
  if (!pageReady) {
    await f.server.close();
    f.db.close();
    assert.fail('OAuth authorization page is missing the Aevra CLI-copy approval UI');
  }
  assert.match(html, /Approve this connection in the local Aevra UI/);
  assert.ok(html.includes(expectedCommand));
  assert.match(html, /id="copy-command"/);
  assert.match(html, /navigator\.clipboard\.writeText/);
  assert.match(html, /JetBrains Mono/);
  assert.match(html, /data-aevra-oauth/);
  assert.match(html, /\.panel\s*\{\s*width:\s*min\(650px,\s*100%\)/);
  ...
  const mcp = await fetch(`${f.base}/mcp`, {
    method: 'POST',
    headers: { ...jsonHeaders(), authorization: `Bearer ${tokens.access_token}` },
    body: '{}',
  });
...
test('static connector Bearer works on /mcp while legacy path remains compatible', async () => {
  ...
  const bearer = await fetch(`${f.base}/mcp`, {
    method: 'POST',
  ...
  const legacy = await fetch(`${f.base}/mcp/${token}`, {
    method: 'POST',
    headers: jsonHeaders(),
    body: '{}',
  });
