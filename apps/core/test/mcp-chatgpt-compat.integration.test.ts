...
function runtimeFixture(){
  ...
  return{
    ...
    runtime:{
      sessions:{
        create(){const session={id:`session-${sessions.size+1}`,...identity};sessions.set(session.id,session);return session;},
...
function oauthFixture(){
  return{
    ...
    verifyAccessToken(token:string){if(token!=='valid-token')throw new Error('invalid token');return identity;},
...
test('post-OAuth modern MCP probe does not masquerade as an authentication failure',async()=>{
  const {runtime}=runtimeFixture();
  const server=new McpIngressServer('127.0.0.1',0,undefined,undefined,()=>false,runtime,undefined,{oauth:oauthFixture()});
  await server.start();
  try{
    const response=await fetch(`${server.url()}/mcp`,{
      method:'POST',
      headers:{...authHeaders,'mcp-protocol-version':'2026-07-28','mcp-method':'server/discover'},
      body:JSON.stringify({jsonrpc:'2.0',id:'discover-1',method:'server/discover',params:{_meta:{'io.modelcontextprotocol/protocolVersion':'2026-07-28','io.modelcontextprotocol/clientInfo':{name:'ChatGPT',version:'1'}}}}),
    });
    assert.equal(response.status,400,'unsupported modern MCP must negotiate/fallback, not trigger OAuth reauthentication');
...
test('authenticated legacy MCP session errors use transport status codes instead of OAuth 401',async()=>{
  ...
  try{
    ...
    const missing=await fetch(`${server.url()}/mcp`,{method:'POST',headers:{...authHeaders,'mcp-protocol-version':'2025-11-25'},body});
    assert.equal(missing.status,400,'missing MCP session is a bad MCP request, not failed authentication');
    const unknown=await fetch(`${server.url()}/mcp`,{method:'POST',headers:{...authHeaders,'mcp-protocol-version':'2025-11-25','mcp-session-id':'expired-session'},body});
    assert.equal(unknown.status,404,'expired/unknown MCP session must cause MCP reinitialization, not OAuth reauthentication');
  }finally{await server.close();}
});
...
test('OAuth bearer remains valid through initialize and tools/list discovery',async()=>{
  ...
  await server.start();
  try{
    const init=await fetch(`${server.url()}/mcp`,{method:'POST',headers:authHeaders,body:JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{protocolVersion:'2025-11-25',capabilities:{},clientInfo:{name:'ChatGPT',version:'1'}}})});
    assert.equal(init.status,200);
    const sid=init.headers.get('mcp-session-id');assert.ok(sid);
    const tools=await fetch(`${server.url()}/mcp`,{method:'POST',headers:{...authHeaders,'mcp-protocol-version':'2025-11-25','mcp-session-id':sid},body:JSON.stringify({jsonrpc:'2.0',id:2,method:'tools/list'})});
