...
function makeServer(
  db: AevraDatabase,
  options: { plainMcpEnabled?: boolean; verifier?: RemoteIdentityVerifier } = {},
) {
  const raw = db.raw();
  ...
  const admission = {
    verify: async (token: string) => {
      ...
      const identity: VerifiedRemoteIdentity = {
  ...
  return {
    server: new McpIngressServer(
      '127.0.0.1',
      0,
      options.verifier as any,
      undefined,
      () => false,
      runtime,
      admission,
      { plainMcpEnabled: options.plainMcpEnabled },
    ),
    repo: connectorsRepo,
  };
}
...
test('connector mode keeps plain /mcp closed while token endpoint works', async () => {
  ...
  await server.start();
  const plain = await fetch(`${server.url()}/mcp`, {
    method: 'POST',
  ...
  const tokenResponse = await fetch(`${server.url()}/mcp/${token}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      jsonrpc: '2.0',
...
test('unknown and revoked tokens get identical 401 bodies', async () => {
  const db = AevraDatabase.open(':memory:');
  const { server, repo } = makeServer(db, { plainMcpEnabled: false });
  await server.start();
  const { connector, token } = repo.create('temp');
  repo.revoke(connector.id);
  const a = await fetch(`${server.url()}/mcp/${token}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {} }),
  });
  const b = await fetch(`${server.url()}/mcp/nope-nope-nope-nope`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {} }),
  });
...
test('non-mcp paths still 404 and health stays open', async () => {
  const db = AevraDatabase.open(':memory:');
  const { server } = makeServer(db, { plainMcpEnabled: false });
  await server.start();
