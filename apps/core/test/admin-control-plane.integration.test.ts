...
test('admin workspace mutations require same-origin request metadata', async () => {
  const hostRoot = mkdtempSync(path.join(os.tmpdir(), 'aevra-admin-origin-'));
  ...
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap,
    api: { workspaces } as any,
  });
  await server.start();
  try {
    const denied = await request(`${server.url()}/api/workspaces`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', origin: 'https://evil.example' },
      body: JSON.stringify({ name: 'A', hostRoot }),
    });
    assert.equal(denied.status, 403);
    const allowed = await request(`${server.url()}/api/workspaces`, {
      method: 'POST',
      headers: {
...
test('approval allow route only arms the ticket and never executes it', async () => {
  ...
  const approvals = {
    approve() {
      approved++;
      return { id: 'req_1', state: 'APPROVED' };
    },
    deny() {
...
test('workspace admission approval is always one-time and never creates an operation permission rule', async () => {
  ...
  const ticket = {
    ...
    actor: 'oauth:ChatGPT',
    sessionId: 'ses_1',
    operation: { family: 'workspace:select', capability: 'files.read' },
  };
  const approvals = {
    status() {
      return ticket;
...
test('admin Cloudflare workflow exposes authenticate and reachability actions', async () => {
  ...
  const cloudflare = {
    ...
    async checkReachability() {
      checked++;
      return { reachable: true, status: 200, message: 'reachable' };
    },
  };
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap,
    api: { cloudflare } as any,
  });
  await server.start();
  ...
  const probe = await request(`${server.url()}/api/cloudflare/test`, {
    method: 'POST',
    headers: { origin: server.url(), 'sec-fetch-site': 'same-origin' },
  });
  assert.equal(probe.status, 200);
  assert.equal(checked, 1);
  assert.deepEqual(await probe.json(), { reachable: true, status: 200, message: 'reachable' });
  await server.close();
});
...
test('admin Cloudflare setup starts a managed tunnel immediately', async () => {
  ...
  await server.start();
  const response = await request(`${server.url()}/api/cloudflare/setup`, {
    method: 'POST',
    headers: {
      origin: server.url(),
      'sec-fetch-site': 'same-origin',
    ...
    body: JSON.stringify({ hostname: 'mcp.example.com', authMode: 'connector' }),
  });
  assert.equal(response.status, 200);
...
test('Cloudflare status resolves legacy no-Access config as connector mode', async () => {
  ...
  const settings = {
    get: (key: string, fallback: any) =>
      key === 'cloudflare.config' ? { hostname: 'mcp.example.com', tunnelId: 'tid' } : fallback,
  };
  const cloudflare = {
    detectCloudflared: async () => ({ found: true, version: 'x' }),
    ownership: () => 'managed',
  };
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap,
  ...
  assert.equal(value.authMode, 'connector');
  assert.equal(value.hostname, 'mcp.example.com');
  await server.close();
});
...
test('admin exposes OAuth pairing requests and local approve/deny actions', async () => {
  ...
  const oauth = {
    ...
    approveAuthorization(id: string) {
      ...
      return { ...pending[0], status: 'approved' };
    },
    denyAuthorization(id: string) {
      calls.push(`deny:${id}`);
      return { ...pending[0], status: 'denied' };
    },
  };
  const audit = { append: (event: any) => calls.push(`audit:${event.operation}:${event.target}`) };
  const server = new AdminServer('127.0.0.1', 0, () => ({ core: 'running' }), {
    bootstrap,
    api: { oauth, audit } as any,
  });
  await server.start();
  const list = await request(`${server.url()}/api/oauth/requests`);
  assert.equal(list.status, 200);
  assert.deepEqual(await list.json(), pending);
  const approve = await request(`${server.url()}/api/oauth/requests/oauth_req_1/approve`, {
    method: 'POST',
    headers: { origin: server.url(), 'sec-fetch-site': 'same-origin' },
  });
  assert.equal(approve.status, 200);
  const deny = await request(`${server.url()}/api/oauth/requests/oauth_req_1/deny`, {
    method: 'POST',
    headers: { origin: server.url(), 'sec-fetch-site': 'same-origin' },
  });
  ...
  assert.deepEqual(calls, [
    'approve:oauth_req_1',
    'audit:oauth.authorize.approve:oauth_req_1',
    'deny:oauth_req_1',
    'audit:oauth.authorize.deny:oauth_req_1',
  ]);
  await server.close();
});
...
test('onboarding state persists through the admin API', async () => {
  ...
  const saved = await request(`${server.url()}/api/onboarding`, {
    method: 'PATCH',
    headers: {
      origin: server.url(),
      'sec-fetch-site': 'same-origin',
...
test('guide manifest exposes the shipped onboarding manual chapters', async () => {
  ...
  assert.ok(
    chapters.some((x) => x.slug === 'connect-chatgpt' && x.file === '04-connect-chatgpt.md'),
  );
