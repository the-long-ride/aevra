...
async function login(server:AdminServer,secret:string){
  const r=await fetch(`${server.url()}/api/local/bootstrap`,{method:'POST',headers:{'x-aevra-control':secret}});
...
test('connector create and revoke emit hash-chained audit events',async()=>{
  ...
  const server=new AdminServer('127.0.0.1',0,()=>({core:'running'}),{bootstrap,controlSecret:'secret',api:{connectors:new ConnectorRepository(raw),audit}});
  await server.start();const cookie=await login(server,'secret');
  const created=await (await fetch(`${server.url()}/api/connectors`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({name:'Audited'})})).json() as any;
  await fetch(`${server.url()}/api/connectors/${created.id}`,{method:'DELETE',headers:{cookie}});
...
test('connector create/list/revoke over admin API; token shown once',async()=>{
  const db=AevraDatabase.open(':memory:');const raw=db.raw();
  const bootstrap=new AdminBootstrapService(raw);
  const server=new AdminServer('127.0.0.1',0,()=>({core:'running'}),{bootstrap,controlSecret:'secret',api:{connectors:new ConnectorRepository(raw)}});
  await server.start();const cookie=await login(server,'secret');
  let r=await fetch(`${server.url()}/api/connectors`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({name:'Claude.ai'})});
  assert.equal(r.status,201);
  const created=await r.json() as any;
  assert.equal(created.name,'Claude.ai');assert.ok(created.token);
  r=await fetch(`${server.url()}/api/connectors`,{headers:{cookie}});
  const listed=await r.json() as any[];
  assert.equal(listed.length,1);assert.equal('token' in listed[0]!,false);
  r=await fetch(`${server.url()}/api/connectors/${created.id}`,{method:'DELETE',headers:{cookie}});
...
test('duplicate connector name returns 409 CONNECTOR_EXISTS',async()=>{
  ...
  const first=await fetch(`${server.url()}/api/connectors`,{method:'POST',headers:{cookie,'content-type':'application/json'},body});
  assert.equal(first.status,201);
  const second=await fetch(`${server.url()}/api/connectors`,{method:'POST',headers:{cookie,'content-type':'application/json'},body});
...
test('empty name is rejected',async()=>{
  ...
  const server=new AdminServer('127.0.0.1',0,()=>({core:'running'}),{bootstrap,controlSecret:'secret',api:{connectors:new ConnectorRepository(raw)}});
  await server.start();const cookie=await login(server,'secret');
  const r=await fetch(`${server.url()}/api/connectors`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({name:'  '})});
