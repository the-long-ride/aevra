...
test('dashboard snapshot aggregates runtime state and active connections',()=>{
  const context:any={
    ...
    oauth:{listPendingAuthorizations:()=>[{id:'oauth_1'}]},
    sessions:{list:()=>[
      {id:'s1',actor:'oauth:ChatGPT',subject:'grant-a',createdAt:'2026-08-18T00:00:00Z',lastActivityAt:'2026-08-18T00:01:00Z',remoteIp:'1.2.3.4',activeLeaseId:'l1',lease:{workspaceId:'w1',capabilities:['files.read']}},
      {id:'s2',actor:'connector:CLI',subject:'conn-1',createdAt:'2026-08-18T00:02:00Z',lastActivityAt:'2026-08-18T00:03:00Z'},
    ]},
    processes:{listLocal:()=>[{id:'p1',ownership:'owned'}]},
    changes:{list:()=>[{id:'c1',state:'OPEN'},{id:'c2',state:'COMMITTED'}]},
    connectors:{list:()=>[{id:'conn-1',name:'CLI',createdAt:'2026-08-17T00:00:00Z',lastUsedAt:'2026-08-18T00:03:00Z'}]},
    workspaces:{listRemote:()=>[{id:'w1',name:'Aevra'}]},
  };
  const snapshot=buildDashboardRuntimeSnapshot(context,{version:'0.5.0',core:'running',worker:'running',mcp:'running'},'2026-08-18T00:00:00Z',new Date('2026-08-18T00:10:00Z'));
  ...
  assert.equal(snapshot.stats.connectors,1);
