...
function sendOAuthJson(res: ServerResponse, status: number, value: unknown) {
  res.setHeader('cache-control', 'no-store');
  send(res, status, value);
}
function remoteIp(req: IncomingMessage) {
...
export class McpIngressServer {
  ...
  async start() {
    this.server = this.options.tls
      ? https.createServer(this.options.tls, (req, res) => void this.handle(req, res))
    ...
    if (a && typeof a !== 'string') this.port = a.port;
  }
  address() {
  ...
  private async handle(req: IncomingMessage, res: ServerResponse) {
    ...
    if (connectorMatch) {
      ...
      if (bearerToken) {
        ...
        if (this.options.plainMcpEnabled === false) {
          this.unauthorized(res);
          return;
        }
        try {
          identity = await this.verifier.verifyRequest(req);
        } catch {
          this.unauthorized(res);
          return;
        }
    ...
    if (!this.runtime) {
      send(res, 501, { error: 'MCP tools not wired' });
      return;
    }
    try {
      ...
      if (req.method === 'DELETE') {
        if (!sid) {
          send(res, 400, { error: 'MCP session id required' });
          return;
        }
        if (!this.sameIdentity(sid, identity)) {
          send(res, 404, { error: 'MCP session not found' });
          return;
        }
        this.runtime.sessions.disconnect(sid);
        res.statusCode = 204;
        res.end();
        return;
      }
      ...
      send(res, (e as any)?.status ?? 400, { error: e instanceof Error ? e.message : String(e) });
    }
  }
  ...
  private async handleOAuth(req: IncomingMessage, res: ServerResponse, url: URL): Promise<boolean> {
    ...
    if (path === '/oauth/authorize' && method === 'GET') {
      try {
        ...
        sendHtml(
          res,
          400,
          `<!doctype html><meta charset="utf-8"><title>Aevra authorization</title><body style="font-family:system-ui;background:#0a0a0a;color:#fff;padding:32px"><h1>Connection request rejected</h1><p>${h(e instanceof Error ? e.message : String(e))}</p></body>`,
        );
      }
      return true;
    }
    ...
    if (path === '/oauth/authorize/continue' && method === 'GET') {
      try {
        ...
        sendHtml(
          res,
          409,
          `<!doctype html><meta charset="utf-8"><title>Aevra authorization</title><body style="font-family:system-ui;background:#0a0a0a;color:#fff;padding:32px"><h1>Authorization unavailable</h1><p>${h(e instanceof Error ? e.message : String(e))}</p></body>`,
        );
      }
      return true;
    }
    ...
    return path.startsWith('/oauth/') || path.startsWith('/.well-known/oauth-')
      ? ((res.statusCode = 404),
        res.setHeader('cache-control', 'no-store'),
        res.end('Not Found'),
        true)
  ...
  private sameIdentity(sessionId: string, identity: VerifiedRemoteIdentity) {
    const s = this.runtime?.sessions.get(sessionId);
    return Boolean(s && s.actor === identity.actor && s.subject === identity.subject);
  }
}
