...
export class McpIngressServer{
  ...
  private async handle(req:IncomingMessage,res:ServerResponse){
    ...
    if(path==='/health'){res.setHeader('content-type','application/json');res.end(JSON.stringify({ok:true}));return;}
  ...
  private async handleOAuth(req:IncomingMessage,res:ServerResponse,url:URL):Promise<boolean>{
    ...
    if((path==='/.well-known/oauth-protected-resource'||path==='/.well-known/oauth-protected-resource/mcp')&&method==='GET'){send(res,200,oauth.protectedResourceMetadata());return true;}
    if(path==='/.well-known/oauth-authorization-server'&&method==='GET'){send(res,200,oauth.authorizationServerMetadata());return true;}
    if(path==='/oauth/register'&&method==='POST'){
      try{const input=await readJson(req);send(res,201,oauth.registerClient(input));}catch(e){send(res,400,{error:'invalid_client_metadata',error_description:e instanceof Error?e.message:String(e)});}return true;
    }
    if(path==='/oauth/authorize'&&method==='GET'){
      ...
      }catch(e){sendHtml(res,400,`<!doctype html><meta charset="utf-8"><title>Aevra authorization</title><body style="font-family:system-ui;background:#0b0d0e;color:#f2f5f3;padding:32px"><h1>Connection request rejected</h1><p>${h(e instanceof Error?e.message:String(e))}</p></body>`);}return true;
    }
    if(path==='/oauth/authorize/status'&&method==='GET'){
      const status=oauth.authorizationStatus(url.searchParams.get('request_id')??'');send(res,200,{status:status.status});return true;
    }
    if(path==='/oauth/authorize/continue'&&method==='GET'){
      try{const result=oauth.continueAuthorization(url.searchParams.get('request_id')??'');res.statusCode=302;res.setHeader('location',result.redirectUrl);res.setHeader('cache-control','no-store');res.end();}catch(e){sendHtml(res,409,`<!doctype html><meta charset="utf-8"><title>Aevra authorization</title><body style="font-family:system-ui;background:#0b0d0e;color:#f2f5f3;padding:32px"><h1>Authorization unavailable</h1><p>${h(e instanceof Error?e.message:String(e))}</p></body>`);}return true;
    }
    if(path==='/oauth/token'&&method==='POST'){
      try{
        ...
        res.setHeader('cache-control','no-store');send(res,200,result);
      }catch(e){res.setHeader('cache-control','no-store');send(res,400,{error:'invalid_grant',error_description:e instanceof Error?e.message:String(e)});}return true;
    }
    if(path==='/oauth/revoke'&&method==='POST'){
      const form=new URLSearchParams(await readText(req));oauth.revoke(form.get('token')??'');res.statusCode=200;res.end();return true;
    }
    return path.startsWith('/oauth/')||path.startsWith('/.well-known/oauth-')?(res.statusCode=404,res.end('Not Found'),true):false;
  }

  private authorizationPage(requestId:string,pairingCode:string){
    const id=h(requestId),code=h(pairingCode);
    return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Authorize Aevra</title><style>body{font-family:system-ui,-apple-system,sans-serif;background:#0b0d0e;color:#f2f5f3;margin:0;min-height:100vh;display:grid;place-items:center}.panel{width:min(520px,calc(100% - 32px));border:1px solid #29302d;border-radius:10px;background:#111416;padding:24px}h1{font-size:22px;margin:0 0 8px}p{color:#a6b0aa;line-height:1.5}.code{font:700 24px ui-monospace,monospace;letter-spacing:.12em;color:#73f2a7;margin:18px 0}.status{font-size:13px;color:#f2f5f3}</style></head><body><main class="panel" data-request-id="${id}"><h1>Authorize Aevra</h1><p>Approve this connection in the local Aevra UI. Confirm the pairing code before allowing access.</p><div class="code">${code}</div><p class="status" id="status">Waiting for local approval...</p></main><script>const id=${JSON.stringify(requestId)};const status=document.querySelector('#status');async function poll(){try{const r=await fetch('/oauth/authorize/status?request_id='+encodeURIComponent(id),{cache:'no-store'});const v=await r.json();if(v.status==='APPROVED'){location.replace('/oauth/authorize/continue?request_id='+encodeURIComponent(id));return}if(v.status==='DENIED'||v.status==='EXPIRED'){status.textContent=v.status==='DENIED'?'Connection denied locally.':'Connection request expired.';return}}catch{}setTimeout(poll,1200)}poll()</script></body></html>`;
  }
