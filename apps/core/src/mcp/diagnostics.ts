...
export class McpDiagnostics {
  ...
  recordMethod(method: unknown) {
    if (typeof method === 'string' && method) this.lastMethod = method;
  }

  recordIdentity(actor: string, sessionId?: string) {
    if (sessionId && this.lastMethod === 'initialize' && this.lastSessionId !== sessionId) {
  ...
  private hint(): McpDiagnosticHint {
    ...
    if (this.initializeCount === 0) return 'traffic-no-initialize';
    if (this.toolCallCount === 0) return 'initialized-no-tools';
    return 'active';
  }
}
