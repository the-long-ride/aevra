    ...
    async start() {
      ...
      try {
        ...
        const bootstrap = new AdminBootstrapService(raw),
          controlSecret = ensureLocalControlSecret(config.stateDir);
        cloudflare =
          deps.cloudflare ??
          new CloudflareManagerImpl(settings, undefined, `https://localhost:${config.mcpPort}`);
        ...
        const oauthBase = cloudflareConfig?.hostname
          ? `https://${cloudflareConfig.hostname}`
        ...
        const issuer =
            process.env.AEVRA_CF_ISSUER ??
            cloudflareConfig?.issuer ??
            settings.get<string>('cloudflare.issuer', ''),
          audience =
            process.env.AEVRA_CF_AUDIENCE ??
            cloudflareConfig?.audience ??
            settings.get<string>('cloudflare.audience', '');
        const authMode =
          process.env.AEVRA_CF_ISSUER && process.env.AEVRA_CF_AUDIENCE
            ? 'access'
            : resolveCloudflareAuthMode(cloudflareConfig ?? { issuer, audience });
        const accessReady = authMode === 'access' && Boolean(issuer && audience);
        const verifier = accessReady
        ...
        watchdog =
          !safeMode && settings.get('cloudflare.config', null)
            ? new TunnelWatchdog(() => cloudflare!.checkReachability(), 60_000).start()
            : undefined;
        await admin.start();
        await mcp.start();
        if (!cloudflareConfig?.hostname) oauth.setPublicBaseUrl(mcp.url());
        if (settings.get('cloudflare.config', null) && cloudflare.ownership() === 'managed')
          await cloudflare.startManagedTunnel();
        started = true;
      } catch (error) {
        await cleanup();
        throw error;
