...
function originalIntent(original:any){
  ...
  if(original.tool==='git_push')return`git push ${clean(`${args.remote??'origin'}/${args.branch??'current'}`,100)}`;
  if(original.tool==='process_start')return`start process ${commandPreview(args.command??args)}`;
...
export function presentApproval(ticket:FrozenOperationTicket):ApprovalPresentation{
  ...
  if(payload.sourceTool==='shell_run'||payload.tool==='shell_run'||family.startsWith('shell:')){const script=payload.script??(Array.isArray(command?.args)?command.args.at(-1):'');return{title:'Run shell script',action:`Run ${clean(payload.shell??family.split(':')[1]??'shell',40)}`,target:executionTarget(args.executionMode??payload.executionMode),...(script?{preview:clean(script,180)}:{})};}
