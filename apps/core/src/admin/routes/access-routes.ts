...
function exposureStatus(context: any) {
  const status = context.exposure?.status?.() ?? { provider: 'local', state: 'stopped' };
  const diagnostics = context.mcpDiagnostics?.() as { state?: string } | undefined;
  ...
  return {
    ...
    health: {
      ...
      tls:
        (localGatewayUrl && localGatewayUrl.startsWith('https://')) ||
...
export const handleAccessRoutes: AdminRouteHandler = async (req, res, url, context) => {
  ...
  if (path === '/api/exposure/status' && method === 'GET') {
    sendAdminResponse(res, 200, exposureStatus(context));
    return true;
  }

  if (path === '/api/exposure/config' && method === 'POST') {
    if (!context.exposure?.configure) {
      throw Object.assign(new Error('Exposure service unavailable'), { status: 503 });
    }
    sendAdminResponse(res, 200, await context.exposure.configure(await readAdminBody(req)));
    return true;
  }

  if (path === '/api/exposure/test' && method === 'POST') {
    if (!context.exposure?.test) {
      throw Object.assign(new Error('Exposure service unavailable'), { status: 503 });
    }
    sendAdminResponse(res, 200, await context.exposure.test());
    return true;
  }
