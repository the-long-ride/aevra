...
import { parseAdminDestination } from './bootstrap-destination.js';
...
export class AdminServer {
  ...
  private async handle(req: IncomingMessage, res: ServerResponse) {
    ...
    if (url.pathname === '/api/local/bootstrap' && req.method === 'POST') {
      if (
        !secretEquals(
      ...
      json(res, 200, await this.options.bootstrap!.issue());
      return;
    }
    ...
    if (url.pathname === '/auth/bootstrap' && req.method === 'GET') {
      ...
      res.setHeader(
        'set-cookie',
        `aevra_admin=${encodeURIComponent(session.sessionId)}; ${this.options.tls ? 'Secure; ' : ''}HttpOnly; SameSite=Strict; Path=/`,
      );
