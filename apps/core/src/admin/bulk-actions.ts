...
const CAPABILITIES=new Set<Capability>(['files.read','files.search','git.read','files.write','files.delete','commands.run','git.commit','git.push','network']);
...
function criticalPersistentRule(effect:string,matcher:string){if(effect!=='allow')return false;const value=matcher.toLowerCase();return /workspace[_:-]?escape|privilege|elevat|security:disable|git:(?:reset|clean|force-push)|git:push.*force/.test(value);}
function ruleKey(rule:any){return[rule.effect,rule.capability,rule.scope,rule.workspaceId??'',rule.sessionId??'',rule.actor??'',rule.matcher].join('\u0000');}
...
export async function handleBulkAdminAction(req:IncomingMessage,res:ServerResponse,url:URL,context:AdminApiContext,currentAdminSession?:string):Promise<boolean>{
  ...
  try{
    ...
    if(path==='/api/permissions/bulk'&&method==='POST'){
      const input=await body(req),effect=input.effect==='deny'?'deny':'allow',scope=String(input.scope??'workspace');
      ...
      if(capabilities.includes('commands.run')&&!commandMatchers.length){bad(res,'Select at least one command matcher');return true;}
      ...
      if(scope==='global'){
        ...
        const workspaceIds=strings(input.workspaceIds);if(!workspaceIds.length){bad(res,'Select at least one workspace');return true;}
        const existing=new Set((context.workspaces?.listRemote?.()??context.workspaces?.listLocal?.()??[]).map((item:any)=>String(item.id)));
        const invalid=workspaceIds.find(id=>!existing.has(id));if(invalid){bad(res,`Unknown workspace: ${invalid}`);return true;}
        for(const actor of actors)for(const workspaceId of workspaceIds)targets.push({actor,workspaceId});
      }else{
        const sessionIds=strings(input.sessionIds);if(!sessionIds.length){bad(res,'Select at least one session');return true;}
        const sessions=(context.sessions?.list?.()??[]) as any[];
        for(const sessionId of sessionIds){const session=sessions.find(item=>item.id===sessionId);if(!session){bad(res,`Unknown session: ${sessionId}`);return true;}if(!actors.includes(String(session.actor))){bad(res,`Session ${sessionId} is not owned by a selected connector`);return true;}targets.push({actor:String(session.actor),sessionId});}
      }

      const expanded:any[]=[];
      for(const target of targets){
        for(const capability of capabilities){
          const matchers=capability==='commands.run'?commandMatchers:[newMatcherMode?'*':legacyMatcher];
          for(const matcher of matchers){
            if(criticalPersistentRule(effect,matcher)){send(res,400,{error:{code:'CRITICAL_RULE_FORBIDDEN',message:'Critical operations cannot receive persistent always-allow rules'}});return true;}
            expanded.push({id:`perm_${randomUUID()}`,effect,capability,scope,...target,matcher,createdAt});
          }
        }
      }
      const seen=new Set<string>(),created=expanded.filter(rule=>{const key=ruleKey(rule);if(seen.has(key))return false;seen.add(key);return true;});
      if(context.permissions?.upsertMany)context.permissions.upsertMany(created);else for(const rule of created)context.permissions?.upsert?.(rule);
      send(res,201,{ok:true,count:created.length,rules:created});return true;
    }
    const admissionMatch=path.match(/^\/api\/workspaces\/([^/]+)\/admissions$/);
    if(admissionMatch&&method==='GET'){send(res,200,context.profiles?.listMappings?.(decodeURIComponent(admissionMatch[1]))??[]);return true;}
    ...
    return false;
  }catch(error){const value=error as any;send(res,value?.status??400,{error:{code:value?.code??'ADMIN_BULK_FAILED',message:error instanceof Error?error.message:String(error)}});return true;}
}
