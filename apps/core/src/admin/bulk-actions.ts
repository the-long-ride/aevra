  ...
  try{
    ...
    if(path==='/api/permissions/bulk'&&method==='POST'){
      ...
      if(scope==='global'){
        ...
        for(const actor of actors)for(const workspaceId of workspaceIds)for(const capability of capabilities){const rule={id:`perm_${randomUUID()}`,effect,capability,scope:'workspace',workspaceId,actor,matcher,createdAt};context.permissions?.upsert?.(rule);created.push(rule);}
      }else{
