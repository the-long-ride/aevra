...
import type { DesktopAppCatalogResult, DesktopCatalogApp } from '../../../../packages/protocol/src/desktop.js';
import { DesktopAppCatalogRepository } from '../../../../packages/store/src/desktop-app-catalog.js';
import { basename, canonicalExecutablePath } from '../../../../packages/security/src/window-gate.js';
...
export class DesktopAppCatalogService {
  ...
  async list(): Promise<DesktopAppCatalogResult> {
    ...
    for (const app of detected.apps) {
      ...
      if (key) apps.set(key, { ...app, sources: [...app.sources], isCustom: false } as DesktopCatalogApp);
    ...
    const catalogApps = [...apps.entries()].map(([key, app]) => {
  ...
  async saveCustom(input: { id?: unknown; executablePath: unknown; displayName?: unknown; version?: unknown }) {
    const executable = normalizeExecutablePath(input.executablePath);
    if (!executable) throw Object.assign(new Error('An absolute Windows .exe path is required'), { code: 'INVALID_REQUEST' });
    let resolved: string;
    try {
      resolved = await realpath(executable);
      if (!(await stat(resolved)).isFile()) throw new Error('not a file');
    } catch {
      throw Object.assign(new Error('The executable path does not point to an existing file on this device'), { code: 'INVALID_REQUEST' });
    }
    if (!resolved.toLowerCase().endsWith('.exe')) {
      throw Object.assign(new Error('The selected file must have an .exe extension'), { code: 'INVALID_REQUEST' });
    }
  ...
  async grant(input: { executablePath: unknown; displayName?: unknown }, decidedBy = 'admin') {
    ...
    if (appBasename(resolved).toLowerCase() === 'msedgewebview2.exe') {
      throw Object.assign(
        new Error('WebView2 is a shared runtime; use the explicitly warned broad process rule or request access to a verified host app'),
    ...
    if (!custom) {
      const detected = await (this.deps.scan ?? scanDesktopApps)();
      const known = detected.apps.some((app) =>
        app.executablePath && canonicalExecutablePath(app.executablePath) === key &&
        app.grantable);
      if (!known) throw Object.assign(new Error('The executable is not present in the verified app catalog'), { code: 'APP_NOT_IN_CATALOG' });
    }
    const name = typeof input.displayName === 'string' && input.displayName.trim()
      ? input.displayName.trim().slice(0, 120)
      : custom?.displayName ?? appBasename(resolved).replace(/\.exe$/i, '');
    return this.deps.access.grantExplicitApp({ executablePath: resolved, displayName: name }, decidedBy);
  }
}
