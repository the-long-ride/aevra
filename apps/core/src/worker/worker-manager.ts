...
const defaultWorkerEntry = () =>
  fileURLToPath(new URL('../../../worker/src/main.js', import.meta.url));
function boundedAppend(current: string, chunk: unknown, max = 8192) {
  const next = current + String(chunk);
  return next.length <= max ? next : next.slice(next.length - max);
}
function workerDiagnostics(stderr: string, stdout: string) {
...
export class WorkerManager {
  ...
  constructor(
    ...
    private processLogDir = path.join(process.cwd(), '.aevra-process-logs'),
  ...
  async start(): Promise<WorkerClient> {
    ...
    if (!existsSync(entry))
      throw new Error(`Execution Worker build is missing at ${entry}; run npm run build`);
    let stdout = '',
      stderr = '',
      exitInfo: ExitInfo | undefined,
      startError: Error | undefined;
    const child = (this.child = spawn(process.execPath, [entry], {
    ...
    const timeout = this.options.startupTimeoutMs ?? 5000,
      poll = this.options.startupPollMs ?? 50,
    ...
    while (Date.now() < deadline) {
      if (startError) {
        ...
        throw new Error(
          `Execution Worker failed to spawn: ${startError.message}${workerDiagnostics(stderr, stdout)}`,
        );
      }
      if (exitInfo) {
        const info = exitInfo;
        await this.cleanupFailedStart();
        throw new Error(
          `Execution Worker exited ${info.code ?? 'null'}${info.signal ? ` (${info.signal})` : ''}${workerDiagnostics(stderr, stdout)}`,
        );
      }
    ...
    await this.cleanupFailedStart();
    const reason =
      lastError instanceof Error ? lastError.message : String(lastError ?? 'startup timeout');
    throw new Error(
