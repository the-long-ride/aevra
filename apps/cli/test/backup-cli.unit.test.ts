...
import {mkdtempSync} from 'node:fs';import path from 'node:path';import {tmpdir} from 'node:os';
import {AevraDatabase} from '../../../packages/store/src/database.js';
import {inspectBackup,restoreBackup} from '../../core/src/backup/verify.js';
function run(file:string){const db=new DatabaseSync(file);db.exec('PRAGMA busy_timeout=5000;');return db;}
test('inspectBackup verifies integrity and reports table counts',()=>{
  ...
  const raw=live.raw();
  raw.prepare('INSERT INTO workspaces(id,name,description,host_root,created_at,updated_at) VALUES(?,?,?,?,?,?)').run('w1','ws','','/tmp/x',new Date().toISOString(),new Date().toISOString());
...
test('restoreBackup seeds a fresh state dir and preserves the previous db',()=>{
  ...
  source.backup(backupPath);source.close();
