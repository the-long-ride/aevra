...
import { validateExposureConfig } from '../../core/src/exposure/config.js';
...
function setupResources(config: ReturnType<typeof loadCoreConfig>) {
  ...
  const cloudflare = new CloudflareManagerImpl(
    settings,
    undefined,
    `https://localhost:${config.publicPort}`,
  );
  const prompt = createInterface({ input, output });
  return {
    prompt,
    cloudflare,
    configure(input: ExposureConfig) {
      settings.set('exposure.config', validateExposureConfig(input));
    },
    close() {
      prompt.close();
      database.close();
