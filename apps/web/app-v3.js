(()=>{
  ...
  function decorateRequestDrawer(approvals){
    ...
    for(const card of document.querySelectorAll('#request-drawer .request-card')){
      ...
      let detail=card.querySelector('.request-detail');if(!detail){detail=document.createElement('div');detail.className='request-detail';card.querySelector('.request-actions')?.before(detail);}detail.innerHTML=`<b>${h(p.action??item.operation?.family??'Operation')}</b><span>${h(p.target??'')}</span>${p.preview?`<code>${h(p.preview)}</code>`:''}${permissionMatcher?`<span class="request-saved-matcher"><strong>Saved matcher</strong><code>${h(permissionMatcher)}</code></span>`:''}`;
      ...
      const session=actions.querySelector('[data-request-approve][data-scope="session"]');if(session)session.textContent='Allow this session';
      const workspace=actions.querySelector('[data-request-approve][data-scope="workspace"]');if(workspace)workspace.textContent='Always in workspace';
      let global=actions.querySelector('[data-request-approve][data-scope="global"]');if(!global){global=document.createElement('button');global.type='button';global.dataset.requestApprove=String(id);global.dataset.scope='global';global.setAttribute('data-scope','global');global.textContent='Always globally';actions.append(global);}else global.textContent='Always globally';
    }
  }
