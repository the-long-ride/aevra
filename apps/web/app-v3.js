(()=>{
  ...
  function suppressLegacyRequestNotifications(){
    if(nativeNotification){
      try{
        function AevraNotification(title,options){
          if(/^Aevra (?:connection|workspace access|approval) request$/i.test(String(title)))return{close(){}};
          return new nativeNotification(title,options);
        }
        Object.defineProperty(AevraNotification,'permission',{get:()=>nativeNotification.permission});
        AevraNotification.requestPermission=(...args)=>nativeNotification.requestPermission(...args);
        window.Notification=AevraNotification;
      }catch{}
    }
  ...
  async function refreshDetailedRequests(){
    ...
    try{
      const [approvals,oauth]=await Promise.all([getJson('/api/approvals'),getJson('/api/oauth/requests')]);
      ...
      const liveApprovals=new Set(pending.map(item=>String(item.id))),liveOauth=new Set(oauth.map(item=>String(item.id)));for(const id of [...seenApprovals])if(!liveApprovals.has(id))seenApprovals.delete(id);for(const id of [...seenOauth])if(!liveOauth.has(id))seenOauth.delete(id);
      decorateRequestDrawer(approvals);
  ...
  function ensureNotificationButton(){
    const header=document.querySelector('#request-drawer aside>header');if(!header||header.querySelector('#enable-browser-notifications'))return;
    const button=document.createElement('button');button.type='button';button.id='enable-browser-notifications';button.className='request-notification-button';button.textContent=notificationButtonText();button.disabled=!nativeNotification||nativeNotification.permission==='granted'||nativeNotification.permission==='denied';
    button.addEventListener('click',async()=>{if(!nativeNotification)return;const permission=await nativeNotification.requestPermission();button.textContent=notificationButtonText();button.disabled=permission!=='default';toast(permission==='granted'?'Browser notifications enabled':'Browser notifications were not enabled',permission==='granted'?'success':'info',4200);});
    header.querySelector('[data-close-requests]')?.before(button);
  }
  ...
  function decorateRequestDrawer(approvals){
    ...
    for(const card of document.querySelectorAll('#request-drawer .request-card')){
      const button=card.querySelector('[data-request-approve],[data-request-deny]');const id=button?.dataset.requestApprove??button?.dataset.requestDeny;if(!id)continue;const item=byId.get(String(id));if(!item)continue;const p=item.presentation??{};
      let detail=card.querySelector('.request-detail');if(!detail){detail=document.createElement('div');detail.className='request-detail';card.querySelector('.request-actions')?.before(detail);}
      detail.innerHTML=`<b>${h(p.action??item.operation?.family??'Operation')}</b><span>${h(p.target??'')}</span>${p.preview?`<code>${h(p.preview)}</code>`:''}`;
      if(item.operation?.family==='workspace:capability-upgrade'){
        const actions=card.querySelector('.request-actions');if(actions&&!actions.dataset.v3Upgrade){actions.dataset.v3Upgrade='true';actions.innerHTML=`<button type="button" data-request-deny="${h(id)}">Deny</button><button type="button" class="primary" data-request-approve="${h(id)}" data-scope="once">Allow</button>`;}
      }
    }
  }
  ...
  function guideButtons(){
    const slugs={ChatGPT:'connect-chatgpt',Claude:'connect-claude',Gemini:'connect-gemini'};
    for(const example of document.querySelectorAll('#page .client-example')){const name=example.querySelector('h3')?.textContent?.trim(),slug=slugs[name];if(!slug||example.querySelector('[data-v3-guide]'))continue;const button=document.createElement('button');button.type='button';button.dataset.v3Guide=slug;button.textContent='Open guide';example.append(button);}
  }
  ...
  async function arrangeOnboarding(){
    const page=document.querySelector('#page');const onboarding=page?.querySelector('.onboarding-panel');if(!page||!onboarding)return;guideButtons();if(onboarding.dataset.v3Layout)return;onboarding.dataset.v3Layout='true';
    let state;try{state=await getJson('/api/onboarding');}catch{return;}
    if(state?.completed){const remote=page.querySelector(':scope > .dashboard-remote');const body=onboarding.querySelector('.onboarding-body');if(remote&&body&&!onboarding.contains(remote)){remote.classList.add('onboarding-block','wide','remote-in-onboarding');body.prepend(remote);}onboarding.open=false;page.append(onboarding);}
  }

  function patchRuntimeStats(snapshot){
    const values={Version:snapshot.status?.version?`v${String(snapshot.status.version).replace(/^v/,'')}`:'—',Uptime:duration(snapshot.uptimeSeconds),'Remote sessions':snapshot.stats.sessions,'Workspace leases':snapshot.stats.workspaceLeases,'Pending requests':snapshot.pending.total,'Managed processes':snapshot.stats.processes,'Open changes':snapshot.stats.openChanges,'Tool calls':snapshot.stats.toolCalls,'Avg tool latency':snapshot.stats.avgToolLatencyMs==null?'—':`${snapshot.stats.avgToolLatencyMs} ms`,Connectors:snapshot.stats.connectors};
    for(const item of document.querySelectorAll('#page .runtime-grid>div')){const label=item.querySelector('span')?.textContent?.trim();if(label&&Object.hasOwn(values,label)){const strong=item.querySelector('strong');if(strong)strong.textContent=String(values[label]);}}
    const recent=[...document.querySelectorAll('#page .recent-grid>div')];const recentValues={Requests:snapshot.pending.total,Sessions:snapshot.stats.sessions,Processes:snapshot.stats.processes,Changes:snapshot.stats.openChanges};for(const item of recent){const label=item.querySelector('span')?.textContent?.trim(),strong=item.querySelector('strong');if(label&&strong&&Object.hasOwn(recentValues,label))strong.textContent=String(recentValues[label]);}
  }
  ...
  suppressLegacyRequestNotifications();
  setInterval(()=>refreshDashboard().catch(()=>{}),2000);
  setInterval(()=>refreshDetailedRequests().catch(()=>{}),2200);
  refreshDashboard().catch(()=>{});refreshDetailedRequests().catch(()=>{});
})();
