...
test('remote MCP has no filesystem-root administration tools', () => {
  for (const name of [
    'workspace_add',
    'workspace_remove',
    'workspace_update',
    ...
    assert.equal((STABLE_TOOL_NAMES as readonly string[]).includes(name), false, name);
});

test('critical operation cannot be authorized by persistent allow rule', () => {
  ...
  repo.upsert({
    ...
    scope: 'global',
    matcher: 'security:disable',
    createdAt: new Date().toISOString(),
  });
  const d = new PermissionEngine(repo).decide({
    capability: 'commands.run',
    matcher: 'security:disable',
    actor: 'a',
    sessionId: 's',
