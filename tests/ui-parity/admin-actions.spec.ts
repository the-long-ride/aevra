...
for (const surface of ADMIN_SURFACES) {
  test(`${surface.name} runs high-value admin mutations`, async ({ page }) => {
    ...
    await page.getByLabel('Effect').first().click();
    ...
    await page.getByLabel('Workspace state').click();
    ...
    await page.getByRole('button', { name: /Managed processes/ }).click();
    ...
    await page
      .getByRole('dialog', { name: 'Rollback change set' })
    ...
    const sandbox = page.getByRole('button', { name: 'Sandbox backend' });
    await sandbox.click();
    ...
    await page
      .getByRole('dialog', { name: 'Connectors' })
      .getByRole('button', { name: 'New connector', exact: true })
      .click();
