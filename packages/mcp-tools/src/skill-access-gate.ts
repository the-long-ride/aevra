...
type AccessResult={granted:true}|{granted:false;result:{status:'approval_pending';requestId:string;expiresInSeconds:number;scope:'session';sources:['user','workspace'];message:string}};

export class SessionSkillAccessGate{
  private grantedSessions=new Set<string>();

  constructor(private inner:SkillReadableMcpService,private sessions:SessionManager,private approvals:ApprovalService){}

  async call(sessionId:string,name:string,args:any={}){
    if(name==='approval_wait'){
      const requestId=String(args?.requestId??'');
      const ticket=this.approvals.status(requestId);
      if(ticket?.operation.family===SKILL_FAMILY)return this.resumeSkillApproval(sessionId,requestId);
  ...
  async resourceRead(sessionId:string,uri:string){
    const access=await this.ensureSkillAccess(sessionId);
    if(!access.granted)throw new AevraToolError('APPROVAL_PENDING','Local skills read access requires local approval',{requestId:access.result.requestId,scope:'session'});
    if(!this.inner.resourceRead)throw new AevraToolError('SKILL_NOT_FOUND','Skills are not configured');
    return this.inner.resourceRead(sessionId,uri);
  }

  promptsList(){return this.inner.promptsList?.()??{prompts:[]};}

  async promptGet(sessionId:string){
    const access=await this.ensureSkillAccess(sessionId);
    if(!access.granted)throw new AevraToolError('APPROVAL_PENDING','Local skills read access requires local approval',{requestId:access.result.requestId,scope:'session'});
    if(!this.inner.promptGet)throw new AevraToolError('INVALID_REQUEST','Skills are not configured');
  ...
  private async ensureSkillAccess(sessionId:string):Promise<AccessResult>{
    ...
    const existing=[...this.approvals.list()].reverse().find(ticket=>ticket.sessionId===sessionId&&ticket.operation.family===SKILL_FAMILY);
    if(existing){
      ...
      if(latest.state==='DENIED')throw new AevraToolError('APPROVAL_DENIED','Local skills read access was denied for this MCP session');
    ...
    const request=await this.approvals.request({
      ...
      operation:{family:SKILL_FAMILY,capability:'files.read',risk:'MEDIUM',argsHash:'session-local-skills-read-v1'},
    ...
    if(!ticket)throw new AevraToolError('APPROVAL_PENDING','Local skills approval request was not found');
  ...
  private async resumeSkillApproval(sessionId:string,requestId:string){
    ...
    if(ticket.operation.family!==SKILL_FAMILY)return this.inner.call(sessionId,'approval_wait',{requestId});
    if(ticket.state!=='APPROVED')return ticket;
    return this.approvals.resume(requestId,async current=>{
      const session=this.sessions.get(sessionId);
      if(!session||session.id!==current.sessionId||session.actor!==current.actor)return{ok:false,reason:'session changed'};
      return{ok:true};
    },async()=>{
      this.grantedSessions.add(sessionId);
      return{status:'skill_access_granted',scope:'session',sources:['user','workspace'] as const};
    });
  }
