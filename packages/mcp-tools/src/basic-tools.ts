...
async function authorizeRead(
  ...
  if (!context.workspaceId && !context.sessions.activeLease(sessionId)) return null;
...
export async function handleBasicTool(
  ...
  if (name === 'aevra_status') {
    ...
    return {
      sessionId,
      workspace: lease
        ? (remote.find((workspace) => workspace.id === lease.workspaceId) ?? null)
        : null,
      workspaces,
