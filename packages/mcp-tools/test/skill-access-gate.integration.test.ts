...
function fixture(){
  ...
  const inner={
    async call(sessionId:string,name:string,args:any={}){calls.push({sessionId,name,args});return{name,args};},
    resourcesList(){return{resources:[{uri:'aevra://skill/user/demo',name:'demo',description:'demo',mimeType:'text/markdown'}]};},
    async resourceRead(_sessionId:string,uri:string){return{uri,contents:[{uri,mimeType:'text/markdown',text:'skill body'}]};},
    promptsList(){return{prompts:[{name:'aevra-instructions',description:'instructions'}]};},
    async promptGet(){return{description:'instructions',messages:[{role:'user',content:{type:'text',text:'AGENTS'}}]};},
  };
  const gate=new SessionSkillAccessGate(inner as any,sessions,approvals);
  const identity={actor:'oauth:ChatGPT',subject:'oauth_grant_one',issuer:'https://example.test',audience:'https://example.test/mcp',expiresAt:new Date(Date.now()+60_000).toISOString()};
...
test('one local approval unlocks all skill and instruction reads for one MCP session',async()=>{
  ...
  assert.deepEqual(f.gate.resourcesList(session.id),{resources:[]},'passive resource discovery must not reveal the local skill inventory or trigger approval');
  ...
  const repeated=await f.gate.call(session.id,'skill_read',{source:'user',name:'demo'}) as any;
  assert.equal(repeated.requestId,first.requestId);
  assert.equal(f.approvals.list().filter(ticket=>ticket.operation.family==='skills:read').length,1);

  f.approvals.approve(first.requestId,'once');
  const granted=await f.gate.call(session.id,'approval_wait',{requestId:first.requestId}) as any;
  ...
  const read=await f.gate.call(session.id,'skill_read',{source:'workspace',name:'demo'}) as any;
  ...
  assert.equal(f.approvals.list().filter(ticket=>ticket.operation.family==='skills:read').length,1,'all skill reads in the session share the same approval');
  assert.equal(f.gate.resourcesList(session.id).resources.length,1);
  assert.equal((await f.gate.resourceRead(session.id,'aevra://skill/user/demo')).contents[0]?.text,'skill body');
...
test('a fresh MCP session requires a new local skills approval even for the same OAuth authorization',async()=>{
  ...
  assert.equal(f.approvals.list().filter(ticket=>ticket.operation.family==='skills:read').length,2);
  f.db.close();
});
...
test('denying local skills access blocks the rest of that MCP session without repeated prompts',async()=>{
  ...
  await assert.rejects(()=>f.gate.call(session.id,'skills_list',{}),(error:any)=>error?.code==='APPROVAL_DENIED');
  await assert.rejects(()=>f.gate.promptGet(session.id),(error:any)=>error?.code==='APPROVAL_DENIED');
  assert.equal(f.approvals.list().filter(ticket=>ticket.operation.family==='skills:read').length,1);
  f.db.close();
});
