...
test('stable tool vocabulary includes read and future policy tools but no root mutation',()=>{
  for(const n of ['aevra_status','workspace_select','file_read','approval_wait','change_rollback'])assert.ok(STABLE_TOOL_NAMES.includes(n as any));
...
test('workspace and file discovery tools are explicitly read-only for ChatGPT filtering',()=>{
  ...
  for(const name of ['file_write','command_run','git_push'])assert.notEqual((definitions.get(name) as any)?.annotations?.readOnlyHint,true,`${name} must not be presented as read-only`);
});
