...
test('read-only shell request preserves env and timeout across coding upgrade before shell approval',async()=>{
  const x=fixture('read-only');await x.service.call(x.session.id,'workspace_select',{workspace:x.workspace.id});const first:any=await x.service.call(x.session.id,'shell_run',{script:'echo "$NAME"',env:{NAME:'aevra'},timeoutMs:4321});assert.equal(first.status,'approval_pending');let ticket=x.approvals.status(first.requestId)!;assert.equal(ticket.operation.family,'workspace:capability-upgrade');assert.equal((ticket.payload as any).profileId,'coding-session');x.approvals.approve(ticket.id,'once');const afterUpgrade:any=await x.service.call(x.session.id,'approval_wait',{requestId:ticket.id});assert.equal(afterUpgrade.status,'approval_pending','after coding access, shell still needs its own HIGH-risk approval');ticket=x.approvals.status(afterUpgrade.requestId)!;assert.equal(ticket.operation.family,'shell:bash');const command=(ticket.payload as any).args.command;assert.equal(command.timeoutMs,4321);assert.equal(command.env.NAME,'aevra');assert.deepEqual(command.args,['-lc','echo "$NAME"']);x.approvals.approve(ticket.id,'once');await x.service.call(x.session.id,'approval_wait',{requestId:ticket.id});assert.equal(x.executions.length,1);assert.equal(x.executions[0][1].timeoutMs,4321);assert.equal(x.executions[0][1].env.NAME,'aevra');x.db.close();
});
