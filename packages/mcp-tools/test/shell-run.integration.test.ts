...
function fixture(
  ...
  profiles.mapActor('oauth:ChatGPT', workspace.id, profile, 'auto');
  const sessions = new SessionManager(new SessionRepository(db.raw()), profiles);
  const session = sessions.create({
    actor: 'oauth:ChatGPT',
    subject: `grant-${profile}`,
  ...
  const settings = {
    get<T>(key: string, fallback: T): T {
      return key === 'execution.settings' ? (executionSettings as T) : fallback;
    },
  };
  const service = new McpToolService(
    ...
    approvals,
    { approvals, operations, settings },
  );
  return { db, workspace, sessions, session, approvals, executions, service };
}
...
test('shell_run requires high-risk local approval and resumes through command_run', async () => {
  ...
  assert.equal(x.executions.length, 0, 'shell must not execute before local approval');
...
test('read-only shell request asks once for the exact commands.run matcher and preserves env and timeout', async () => {
  const x = fixture('read-only');
  await x.service.call(x.session.id, 'workspace_select', { workspace: x.workspace.id });
  const first: any = await x.service.call(x.session.id, 'shell_run', {
    script: 'echo "$NAME"',
    env: { NAME: 'aevra' },
    timeoutMs: 4321,
  });
  ...
  assert.equal(ticket.operation.family, 'shell:bash:*');
  assert.equal(ticket.operation.capability, 'commands.run');
  assert.equal(payload.tool, 'capability_request');
  assert.equal(payload.requestedCapability, 'commands.run');
  assert.equal(payload.permissionMatcher, 'shell:bash:*');
  ...
  assert.ok(
    !x.sessions.activeLease(x.session.id)?.capabilities.includes('commands.run'),
    'exact approval must not mutate the read-only profile',
  );
  x.db.close();
});

test('native execution setting resolves an unspecified shell to host before approval matching', async () => {
  const x = fixture('developer', { sandboxBackend: 'native' });
  await x.service.call(x.session.id, 'workspace_select', { workspace: x.workspace.id });
  const pending: any = await x.service.call(x.session.id, 'shell_run', { script: 'pwd' });
  ...
  await x.service.call(x.session.id, 'approval_wait', { requestId: ticket.id });
  assert.equal(x.executions.length, 1);
  assert.equal(x.executions[0][2], 'host');
  x.db.close();
});

test('explicit sandbox execution wins over the native host default', async () => {
  const x = fixture('developer', { sandboxBackend: 'native' });
  await x.service.call(x.session.id, 'workspace_select', { workspace: x.workspace.id });
  const pending: any = await x.service.call(x.session.id, 'shell_run', {
    script: 'pwd',
