...
test('native multi-search runs values together and returns safe hits', async () => {
  ...
  try {
    ...
    assert.equal(
      result.results[0]?.hits[0]?.path,
      '/alpha.ts',
      JSON.stringify(result.results[0]),
    );
    assert.equal(result.results[0]?.hits[0]?.line, 1);
    assert.equal(
      result.results[1]?.hits[0]?.path,
      '/notes.md',
      JSON.stringify(result.results[1]),
    );
  } finally {
...
test('node fallback backend scans bounded, skips .git, and matches every mode', async () => {
  ...
  try {
    ...
    const regex = await nodeCandidates({ value: 'need[e]+', mode: 'regex', path: '/' }, root);
    ...
    assert.deepEqual(files.candidates.map((candidate) => path.basename(candidate.path)), [
      'readme.md',
    ]);
  } finally {
    await rm(root, { recursive: true, force: true });
