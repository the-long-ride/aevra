...
export class ProcessRepository {
  ...
  put(process: any) {
    ...
    this.db
      .prepare(
        `INSERT OR REPLACE INTO managed_processes(id,name,workspace_id,lifecycle,ownership,helper_pid,helper_started_at,marker,command_json,execution_mode,log_path,created_at,updated_at,state,exit_code,signal,finished_at,failure_message) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        process.id,
        process.name ?? null,
  ...
  updateStatus(status: ManagedProcessStatus) {
    ...
    this.db
      ...
      .run(status.state, status.exitCode, status.signal, status.finishedAt, now, status.processId);
