...
export class DesktopAppCatalogRepository {
  ...
  listCustom(): DesktopCustomAppRecord[] {
    return this.db
      .prepare(
        `SELECT ${columns} FROM desktop_custom_apps
      ORDER BY display_name COLLATE NOCASE`,
      )
      .all() as unknown as DesktopCustomAppRecord[];
  }
  ...
  saveCustom(input: SaveDesktopCustomApp): DesktopCustomAppRecord {
    ...
    const previous = id
      ? (this.db
          .prepare(`SELECT ${columns} FROM desktop_custom_apps WHERE id=?`)
          .get(id) as unknown as DesktopCustomAppRecord | undefined)
      : undefined;
    if (previous) {
      this.db
        .prepare(
          `UPDATE desktop_custom_apps SET path_key=?,executable_path=?,display_name=?,
    ...
    this.db
      .prepare(
        `INSERT INTO desktop_custom_apps(
    ...
    return this.db
      .prepare(`SELECT ${columns} FROM desktop_custom_apps WHERE path_key=?`)
