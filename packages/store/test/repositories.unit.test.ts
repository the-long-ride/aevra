...
import { ChangeRepository } from '../src/changes.js';
...
test('managed process status reconciliation preserves ownership and command metadata', () => {
  ...
  assert.equal(repo.list('ws').length, 1);
  ...
  const minRec = repo.get('proc_min');
  assert.equal(minRec.state, 'running');
  assert.equal(minRec.helper_pid, null);

  repo.markKeepRunningUncertain();
  repo.delete('proc_1');
  repo.delete('proc_min');
  assert.equal(repo.get('proc_1'), undefined);
...
test('permission repository CRUD and batch operations', () => {
  const db = AevraDatabase.open(':memory:');
  const raw = db.raw();
  const repo = new PermissionRepository(raw);
  ...
  assert.equal(repo.get('p1').id, 'p1');
  assert.equal(repo.list().length, 1);
  ...
  repo.expireSession('sess-1');
  assert.equal(repo.list().length, 0);
  db.close();
});

test('change repository put and record', () => {
  const db = AevraDatabase.open(':memory:');
  const raw = db.raw();
  const now = new Date().toISOString();
  raw
    ...
    .run(now, now);
  const repo = new ChangeRepository(raw);
  ...
  const row = raw
    .prepare('SELECT * FROM change_operations WHERE change_set_id=?')
    .get('cs1') as any;
  assert.equal(row.operation_id, 'op1');
  ...
  const row2 = raw
    .prepare('SELECT * FROM change_operations WHERE change_set_id=?')
    .get('cs2') as any;
  assert.equal(row2.operation_id, 'op2');
  assert.equal(row2.logical_path, null);
