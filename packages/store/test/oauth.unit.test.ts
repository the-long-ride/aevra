...
test('clientFromRow safely handles malformed JSON and defaults', async () => {
  const { clientFromRow } = await import('../src/oauth-records.js');
  const client = clientFromRow({
    clientId: 'c1',
    clientName: 'Test',
    redirectUrisJson: 'invalid-json',
    grantTypesJson: null,
    responseTypesJson: '["code"]',
    createdAt: '2026-08-25',
  });
  assert.equal(client.clientId, 'c1');
  assert.deepEqual(client.redirectUris, []);
  assert.deepEqual(client.grantTypes, []);
  assert.deepEqual(client.responseTypes, ['code']);

  // Test denyAuthorizationRequest and expired getAuthorizationRequest
  const db = AevraDatabase.open(':memory:');
  let currentTime = Date.parse('2026-08-25T12:00:00.000Z');
  const repo = new OAuthRepository(db.raw(), () => new Date(currentTime));
  const c = repo.registerClient({ clientName: 'C', redirectUris: ['https://c.com'] });
  const req1 = repo.createAuthorizationRequest(
    {
      clientId: c.clientId,
      redirectUri: 'https://c.com',
      scope: 'mcp',
      resource: 'https://mcp',
      codeChallenge: 'ch1',
      codeChallengeMethod: 'S256',
  ...
  const denied = repo.denyAuthorizationRequest(req1.id);
  assert.equal(denied?.status, 'DENIED');

  const req2 = repo.createAuthorizationRequest(
    {
      clientId: c.clientId,
      redirectUri: 'https://c.com',
      scope: 'mcp',
  ...
  currentTime += 5_000;
  assert.equal(repo.getAuthorizationRequest(req2.id), null);
  db.close();
});
