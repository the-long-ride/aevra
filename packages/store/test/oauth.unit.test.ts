...
test('OAuth repository stores client metadata and hashes bearer credentials',()=>{
  ...
  const client=repo.registerClient({clientName:'ChatGPT',redirectUris:['https://chatgpt.com/oauth/callback']});
  assert.match(client.clientId,/^oauth_client_/);
  assert.deepEqual(client.redirectUris,['https://chatgpt.com/oauth/callback']);

  const access=repo.issueAccessToken({clientId:client.clientId,actor:'oauth:ChatGPT',subject:client.clientId,scope:'mcp offline_access',resource:'https://mcp.example.com/mcp'},60_000);
  const refresh=repo.issueRefreshToken({clientId:client.clientId,actor:'oauth:ChatGPT',subject:client.clientId,scope:'mcp offline_access',resource:'https://mcp.example.com/mcp'},120_000);
  assert.ok(repo.findAccessToken(access.token));
  assert.ok(repo.findRefreshToken(refresh.token));

  const stored=JSON.stringify(db.raw().prepare('SELECT * FROM oauth_access_tokens').all())+JSON.stringify(db.raw().prepare('SELECT * FROM oauth_refresh_tokens').all());
...
test('authorization codes are single-use and pending state is restart-invalidated',()=>{
  ...
  const client=repo.registerClient({clientName:'ChatGPT',redirectUris:['https://chatgpt.com/oauth/callback']});
  const pending=repo.createAuthorizationRequest({clientId:client.clientId,redirectUri:client.redirectUris[0]!,scope:'mcp offline_access',resource:'https://mcp.example.com/mcp',codeChallenge:'challenge',codeChallengeMethod:'S256',state:'abc',remoteIp:'203.0.113.5'},60_000);
  ...
  repo.createAuthorizationRequest({clientId:client.clientId,redirectUri:client.redirectUris[0]!,scope:'mcp',resource:'https://mcp.example.com/mcp',codeChallenge:'challenge2',codeChallengeMethod:'S256',remoteIp:'203.0.113.5'},60_000);
  const second=repo.createAuthorizationRequest({clientId:client.clientId,redirectUri:client.redirectUris[0]!,scope:'mcp',resource:'https://mcp.example.com/mcp',codeChallenge:'challenge3',codeChallengeMethod:'S256',remoteIp:'203.0.113.5'},60_000);
  ...
  assert.equal(Number((db.raw().prepare('SELECT COUNT(*) count FROM oauth_authorization_codes').get() as any).count),0);
  db.close();
});
...
test('access expiry and refresh rotation reject stale credentials',()=>{
  ...
  const repo=new OAuthRepository(db.raw(),()=>new Date(now));
  const client=repo.registerClient({clientName:'ChatGPT',redirectUris:['https://chatgpt.com/oauth/callback']});
  const grant={clientId:client.clientId,actor:'oauth:ChatGPT',subject:client.clientId,scope:'mcp offline_access',resource:'https://mcp.example.com/mcp'};
