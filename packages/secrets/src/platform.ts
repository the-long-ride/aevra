...
export class SpawnAdapter implements ProcessAdapter {
  async run(file: string, args: string[], input?: string) {
    return await new Promise((resolve, reject) => {
      ...
      c.once('error', reject);
      c.once('exit', (code) => resolve({ code: code ?? 1, stdout }));
...
export class CommandSecretStore implements SecretStore {
  ...
  async set(ref: string, value: string) {
    if (this.platform === 'darwin') {
      const r = await this.adapter.run('security', [
      ...
      if (r.code) throw new Error('Keychain store failed');
      return;
    }
    if (this.platform === 'linux') {
      ...
      if (r.code) throw new Error('Secret Service store failed');
      return;
    }
    if (this.platform === 'win32') {
      ...
      const r = await this.adapter.run('powershell.exe', [
        '-NoProfile',
        '-NonInteractive',
        '-Command',
        script,
      ]);
      if (r.code) throw new Error('Credential Manager store failed');
      return;
    }
  ...
  async get(ref: string) {
    ...
    if (this.platform === 'linux') {
      const r = await this.adapter.run('secret-tool', [
        'lookup',
        'service',
        this.service,
        'ref',
