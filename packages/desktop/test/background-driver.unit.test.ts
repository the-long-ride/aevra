...
test('mapBackgroundFailure distinguishes pre-dispatch and post-dispatch uncertainty', () => {
  ...
  const genericPost = mapBackgroundFailure(new Error('Network disconnected'), true) as DesktopDriverError;
...
test('executeBackgroundAction dispatches invoke, setValue, select, toggle and invalidates snapshot', async () => {
  for (const op of ['invoke', 'setValue', 'select', 'toggle'] as const) {
    ...
    const mockDriver: DesktopDriver = {
      connect: async () => ({ capture: true, tree: true, attribution: true, input: true, backgroundActions: true }),
      windows: async () => [],
      ...
      act: async () => ({ ok: true, delta: { focusChanged: false, newWindow: false, subtreeChanged: false } }),
      disconnect: async () => {},
    ...
    const target =
      op === 'setValue'
        ? { windowId: 'win1', windowLeaseId, snapshotId: 'snap1', ref: 'ref1', op: 'setValue' as const, value: 'hello' }
    ...
    assert.throws(
      () => state.resolve(TEST_OWNER, { windowId: 'win1', windowLeaseId, snapshotId: 'snap1', ref: 'ref1' }, 1),
...
test('executeBackgroundAction maps driver timeout after dispatch to DESKTOP_OUTCOME_UNKNOWN and invalidates snapshot', async () => {
  ...
  const hangingDriver: DesktopDriver = {
    ...
    backgroundAct: async () => {
      dispatchCount++;
      throw new DesktopDriverError('DESKTOP_DRIVER_DEADLINE', 'Helper call timed out: backgroundAct');
    },
  };
...
test('executeBackgroundAction suspends lease on observed focus change and refuses subsequent actions', async () => {
  ...
  assert.throws(
    () => state.resolve(TEST_OWNER, { windowId: 'win1', windowLeaseId, snapshotId: 'snap2', ref: 'ref2' }, 1),
  ...
  const resolved = state.resolve(TEST_OWNER, { windowId: 'win1', windowLeaseId, snapshotId: 'snap2', ref: 'ref2' }, 1);
